...
transFieldNames2Generator = {
    '业务标识': getRandEltGenerator(genRandList(2)),
    '核心主键': getRandStrGenerator(15),
    '流水号': getRandStrGenerator(15),
    '传票号': getRandStrGenerator(15),
    '交易机构': getRandEltGenerator(genRandList(2)),
    '交易日期': getRandDateGenerator(),
    '交易时间': getRandDateGenerator(),
    '我行账户': getRandStrGenerator(15),
    # '我行客户号': getRandStrGenerator(15),
    '客户类型': getRandEltGenerator(genRandList(2)),
    '交易代码': getRandEltGenerator(genRandList(2)),
    '原交易代码': getRandEltGenerator(genRandList(2)),
    '业务类型': getRandEltGenerator(genRandList(2)),
    '交易类型': getRandEltGenerator(genRandList(2)),
    '借贷标志': getRandBoolGenerator(),
    '收付标志': getRandBoolGenerator(),
    '科目号': getRandEltGenerator(genRandList(2)),
    '币种': getRandEltGenerator(genRandList(2)),
    ...
    '结算方式': getRandEltGenerator(genRandList(2)),
    '用途': getRandStrGenerator(15),
    '对方系统': getRandEltGenerator(genRandList(2)),
    '对方是否我行客户': getRandBoolGenerator(),
    '对方所在地区': getRandEltGenerator(genRandList(2)),
    '对方行号类型': getRandEltGenerator(genRandList(2)),
    ...
    '对方账户类型': getRandEltGenerator(genRandList(2)),
    '对方证件类型': getRandEltGenerator(genRandList(2)),
    '对方证件号码': getRandStrGenerator(15),
    '对方客户类型': getRandEltGenerator(genRandList(2)),
    ...
    '处理状态': getRandEltGenerator(genRandList(2)),
    '当事人中文名称': getRandStrGenerator(15),
    '是否已补录': getRandBoolGenerator(),
    '补录时间': getRandDateGenerator(),
    '对方卡号': getRandStrGenerator(15),
    '凭证代码': getRandEltGenerator(genRandList(2)),
    '原客户号': getRandStrGenerator(15),
    '交易渠道': getRandEltGenerator(genRandList(2)),
    '是否计算': getRandBoolGenerator(),
    '是否规则': getRandBoolGenerator(),
    '事件类型': getRandEltGenerator(genRandList(2)),
    '对方金融机构国家': getRandEltGenerator(genRandList(2)),
    '网银IP地址': getRandStrGenerator(15),
    '交易去向国别': getRandEltGenerator(genRandList(2)),
    '交易去向行政区': getRandEltGenerator(genRandList(2)),
    '交易发生地国别': getRandEltGenerator(genRandList(2)),
    '交易发生地行政区': getRandEltGenerator(genRandList(2)),
    '代办人姓名': getRandStrGenerator(15),
    '代办人身份证件/证明文件类型': getRandEltGenerator(genRandList(2)),
    '代办人身份证件/证明文件号码': getRandStrGenerator(15),
    '代办人国籍': getRandEltGenerator(genRandList(2)),
    '金融机构和交易关系': getRandEltGenerator(genRandList(2)),
    '现钞标志': getRandBoolGenerator(),
    '对方人行客户类型': getRandEltGenerator(genRandList(2)),
    '交易卡号': getRandStrGenerator(15),
    '上次更新柜员': getRandStrGenerator(15),
    '对方是否离岸账户': getRandBoolGenerator(),
    '业务代号': getRandStrGenerator(15),
    '大额验证状态': getRandEltGenerator(genRandList(2)),
    '可疑验证状态': getRandEltGenerator(genRandList(2)),
